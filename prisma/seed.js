...
const main = async () => {
...

  {
    ...
    const consumptionMethodsData = enumMethods.map((method, index) => ({
      method: method,
      isActive: true,
      displayOrder: index + 1, // Garante a ordem: DELIVERY=1, PICKUP=2, DINE_IN=3
    }));
    ...
    console.log(
      "Criando lista de ingredientes adicionais para Congo Burger...",
    );
...

  {
    ...
    const consumptionMethodsData = enumMethods.map((method, index) => ({
      method: method,
      isActive: true,
      displayOrder: index + 1, // Garante a ordem: DELIVERY=1, PICKUP=2
    }));
    ...
    const pizzaRestaurant = await prismaClient.restaurant.create({
      data: {
        ...
        category: "PIZZARIA",
        consumptionMethods: { create: consumptionMethodsData },
        latitude: -23.5505,
        longitude: -46.6333,
        socialMedia: {
    ...
    console.log(
      "Criando lista de ingredientes adicionais para pizzas doces...",
    );
    await prismaClient.additionalIngredient.createMany({
      data: [
        {
