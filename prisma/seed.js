...
const main = async () => {
  await prismaClient.$transaction(async (tx) => {
    ...
    // Cria ou reutiliza o dono do restaurante
    const owner = await tx.user.upsert({
      where: { email: "owner@congo-burger.com" },
      update: {},
      create: {
        email: "owner@congo-burger.com",
        password: "dev-password", // ajuste para sua estratégia real de hashing
        name: "Dono Congo Burger",
        role: "RESTAURANT_OWNER",
      },
    });

    // Cria o restaurante com campos obrigatórios do schema
